/* global charitable_builder, charitable_campaign_builder_field_conditionals, wpchar, jconfirm, charitable_panel_switch, Charitable, CharitableCampaignEmbedWizard, CharitableCampaignCongratsWizard, wpCookies, tinyMCE, CharitableUtils */
	...
	var app = {
		...
		showUpgradeBanner: function() {
			...
			if ( $templates.length > 5 ) {
				$ ('#charitable-template-list .charitable-template-list-container > div:eq(5)').after( template() );
				return;
			}
		...
		initTemplatePanel: function() {
			...
			// feedback form.
			elements.$templatePreview.on( 'click', '.charitable-template-lite-to-pro a.button, .charitable-setup-desc.secondary-text a.send-feedback', function( e ) {
				e.preventDefault();
		...
		bindAutoResizeCampaignTitle: function() {
			...
			$builder.on( 'input', 'input#charitable_settings_title', function( e ) { // eslint-disable-line
				...
				var $this = $( this ),
					...
					the_padding = 10,
					the_input_box_width = ( the_text_characters * 10.8 ) + the_padding;
		...
		updateHeadlinePreview: function( field_id = 0, textFieldName = '', label_value = '', theTextBox ) { // eslint-disable-line
			...
			const 	preview_field        = $('#charitable-field-' + field_id ),
					...
					placeholderContainer = preview_field.find('.charitable-placeholder').length > 0 ? '.charitable-placeholder' : '.placeholder';
		...
		initSuggestedDonations: function ( $element ) {
			...
			var $table             = $element.closest( '.charitable-campaign-suggested-donations' ),
				...
				donation_type      = $add_row_button.data( 'charitable-add-row' ), // this can be most likely "suggested-amount" or "suggested-recurring-amount"
			...
			$add_row_button.on( 'click', function( event ) {
				...
				var type = $( this ).data( 'charitable-add-row' ); // this can be most likely "suggested-amount" or "suggested-recurring-amount"
		...
		*/
		 ...
		 * @param {string} type This can be most likely "suggested-amount" or "suggested-recurring-amount"
		 */
		add_suggested_amount_row: function( $button, type = '' ) { // eslint-disable-line
			...
			var className = 'charitable-campaign-suggested-donations-table';

			switch ( type ) {
				case 'recurring':
				case 'suggested-recurring-amount':
					className = 'charitable-campaign-suggested-recurring-donations-table';
					break;
			}

			// Go through the donations table instances, in settings or not.
			$builder.find( '.' + className ).each( function() {
		...
		delete_suggested_amount_row: function( $button, type = '' ) {
			...
			if ( row_to_delete_index > 0 ) {
				...
				wpchar.debug( $('.charitable-campaign-suggested-donations-table tbody tr[data-index="' + row_to_delete_index + '"]') );
				wpchar.debug( '.charitable-campaign-suggested-donations-table tbody tr[data-index="' + row_to_delete_index + '"]' );

				$builder.find( '.' + className ).each( function() {
					// remove the row in all instances of the donations table.
					$( this ).find('tbody tr[data-index="' + row_to_delete_index + '"]').remove();
				});
