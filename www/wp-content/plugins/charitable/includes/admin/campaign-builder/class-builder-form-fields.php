...
class Charitable_Builder_Form_Fields {
	...
	public function generate_text( $value = false, $label = false, $args = false ) {
		...
		if ( $icon ) {
			$html .= '<div class="charitable-text-field-icon"><img src="' . $icon . '" /></div>';
		}
	...
	public function generate_checkboxes( $value = false, $label = false, $args = false ) {
		...
		foreach ( $params['options'] as $text => $box_id ) {
			...
			$checked = ( ! empty( $defaults ) && in_array( $box_id, $params['defaults'] ) ) || ( $value !== false && key_exists( $box_id, $value ) && $value[ $box_id ] !== false ) ? 'checked="checked"' : false;
	...
	public function generate_dropdown( $value = false, $label = false, $args = false ) {
		...
		if ( false !== $params['options'] && is_array( $params['options'] ) && count( $params['options'] ) > 0 ) {
			...
			foreach ( $params['options'] as $option_value => $text ) {
				...
				// $selected = ( isset( $selected_value ) && $selected_value !== false && $option_value === $selected_value ) ? 'selected="selected"' : false;
	...
	public function generate_textbox( $value = false, $label = false, $args = false ) {
		...
		if ( $args['html'] ) {
			...
			$html = '<div id="' . $this->id_slug . '-' . $params['id'] . '-wrap" class="charitable-panel-field charitable-panel-field-textarea ' . $params['container_classes'] . '" ' . $field_id_attr . ' data-special-type="' . $params['special'] . '">
                        ...
                        <input type="hidden" value="' . $value . '" name="' . $params['name'] . '" />
                    </div>';

		} elseif ( $args['code'] ) {

			$html = '<div id="' . $this->id_slug . '-' . $params['id'] . '-wrap" class="charitable-panel-field  charitable-panel-field-textarea" ' . $field_id_attr . '>
                        <label for="' . $this->id_slug . '-' . $params['id'] . '">' . $label . ' ' . $tooltip_html . '</label>' . '</label>
		...
		if ( $params['creator_id'] ) :
			...
			<div id="campaign-creator" class="charitable-campaign-creator-info-container">
				...
				<div class="charitable-campaign-creator-info">
					...
					<p class="joined-on"><?php printf( '%s <span>%s</span>', _x( 'Joined on', 'joined on date', 'charitable-ambassadors' ), date_i18n( 'F Y', strtotime( $creator->user_registered ) ) ); ?></p>
					<ul>
						<li><a target="_blank" class="public-profile-link" href="<?php echo get_author_posts_url( $creator->ID ); ?>"><?php _e( 'Public Profile', 'charitable' ); ?></a></li><li><a target="_blank" class="edit-profile-link" href="<?php echo admin_url( 'user-edit.php?user_id=' . $creator->ID ); ?>"><?php _e( 'Edit Profile', 'charitable' ); ?></a></li>
					</ul>
