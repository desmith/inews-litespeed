	...
	class Charitable_Field_Donate_Button extends Charitable_Builder_Field {
		...
		public function field_display( $field, $field_data = false, $campaign_data = false, $is_preview_page = false ) {
			...
			if ( $campaign->has_ended() ) {
				...
				charitable_template_campaign_finished_notice( $campaign );

			} elseif ( ! $campaign->can_receive_donations() && ! $is_preview_page ) {

				do_action( 'charitable_campaign_builder_cannot_receive_donations_notice', $campaign );

			} elseif ( 'modal' === $donation_form_display ) {
				?>
