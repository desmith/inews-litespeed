	...
	class Charitable_Campaign_Builder_Templates {
		...
		public function render_fields( $fields, $theme, $campaign_data, $last_field_id = 0 ) {
			...
			foreach ( $fields as $key => $field_type_data ) :
				...
				if ( class_exists( $type ) ) :
					...
					echo '<div class="' . implode( ' ', $charitable_field_css_classes ) . '" id="charitable-field-' . intval( $field_id ) . '" data-field-id="' . intval( $field_id ) . '" data-field-type="' . esc_attr( $field_type ) . '" data-field-max="' . intval( $class->max_allowed ) . '" style="">';
					if ( $class->can_be_edited ) :
						echo '<a href="#" class="charitable-field-edit" data-type="' . esc_attr( $class->edit_type ) . '" data-section="' . $class->edit_section . '" data-edit-field-id="' . $class->edit_field_id . '" title="' . $class->edit_label . '"><i class="fa fa-pencil"></i></a>';
					endif;
		...
		public function get_template_tab_content( $tabs = false, $theme = false, $campaign_data = array(), $row_fields = false, $last_field_id = 0 ) {
			...
			<div class="tab-content <?php echo $no_tab_class; ?>">
									...
									foreach ( $fields as $key => $field_info_or_id ) :
										...
										if ( false !== $field_type ) :
											...
											if ( class_exists( $class_type ) ) :
												...
												echo '<div class="' . implode( ' ', $charitable_field_css_classes ) . '" id="charitable-field-' . intval( $field_id ) . '" data-field-id="' . intval( $field_id ) . '" data-field-type="' . $field_type . '" data-field-max="' . $class->max_allowed . '" style="">';
												...
												echo $class->field_preview( $field_settings, $campaign_data, $field_id, 'preview', $theme );
												echo '</div>';
					...
					<?php if ( $no_tab_class ) : ?>
						<p class="no-tab-notice"><?php echo $this->no_tab_empty_notice(); ?></p>
					<?php endif; ?>
				</div>
		...
		public function render_missing_addon_field_preview( $field_type = false, $field_id = false, $field_type_data = false, $campaign_data = false ) {
			...
			if ( charitable_is_pro() ) {
				...
				if ( ! empty( $display_html['buttons'] ) ) {
					echo $display_html['buttons'];
				} else {
					echo '<a class="button-link" target="_blank" href="' . admin_url( 'admin.php?page=charitable-addons&search=' . $search_term ) . '">' . esc_html__( 'View Addons', 'charitable' ) . '</a>';
				}
		...
		public function get_addon_install_or_activate_buttons( $field_type = false ) {
			...
			if ( isset( $installed_plugins[ $plugin_basename ] ) && ! is_plugin_active( $plugin_basename ) ) {
			...
			} elseif ( ! in_array( 'charitable-' . $slug . '/charitable-' . $slug . '.php', $installed_plugins, true ) ) {
				...
				return array(
					...
					'install_url' => ! empty( $addon_informationp['install'] ) ? esc_url( $addon_informationp['install'] ) : false,
				);
			}
			...
			<?php } ?>
				...
				if ( ! empty( $layout ) && is_array( $layout ) ) {
					...
					foreach ( $layout as $row ) {
							...
							foreach ( $column as $section ) {
								...
								switch ( $section['type'] ) {
									...
									case 'header':
										...
										break;
									case 'tabs':
										echo $this->get_template_element_start( 'tabs', null, null, $campaign_data );
						...
						echo $this->get_template_element_end( $row['type'], $element_counter );
								...
								foreach ( $section['fields'] as $key => $field_type_data ) :
									...
									if ( class_exists( $type ) ) :
										...
										if ( method_exists( $class, 'settings_display' ) ) {
											echo $class->settings_display( $field_id, $campaign_data ); // second param was $this->campaign_data, but that isn't available.
										}
									endif;
									++$field_id;
								endforeach;
			...
			?>
				...
				<div data-field-id="" id="charitable-group-row-design-layout-options-tabs-enable" class="charitable-panel-field charitable-panel-field-toggle " data-ajax-label="enable_tabs">
					<span class="charitable-toggle-control">
						...
						<?php echo charitable_get_tooltip_html( esc_html__( 'Remove tabs from your public campaign page but not delete any items in the preview.', 'charitable' ), 'tooltipstered' ); ?>
					</span>
				</div>
		 ...
		 * @return array
		 */
		public function get_template_data( $template_id = false, $campaign_data = array() ) {

			if ( false === $template_id ) {
				$template_id = charitable_campaign_builder_default_template();
			}

			$templates_data = $this->get_templates_data( $template_id, $campaign_data );
		 ...
		 * @return array
		 */
		public function get_templates_data( $template_id = false, $campaign_data = array() ) {

			$templates_data = ( defined( 'CHARITABLE_DEBUG' ) && CHARITABLE_DEBUG ) || ( defined( 'CHARITABLE_BUILDER_NO_CACHE_TEMPLATE' ) && CHARITABLE_BUILDER_NO_CACHE_TEMPLATE ) ? false : get_option( 'charitable_campaign_builder_templates' );
		...
		public function output_templates_panel() {
			...
			$create_update_term  = isset( $_GET['campaign_id'] ) && 0 !== intval( $_GET['campaign_id'] ) ? esc_html__( 'Restart Campaign', 'charitable' ) : esc_html__( 'Create Campaign', 'charitable' );
			$current_template_id = false;

			if ( isset( $_GET['campaign_id'] ) && 0 !== intval( $_GET['campaign_id'] ) ) {

				$template_data       = charitable_get_template_data_from_campaign( abs( $_GET['campaign_id'] ) );
				...
				<div id="charitable-template-list" class="charitable-template-list"> <!-- start template list -->
					...
					if ( ! empty( $this->prepared_templates ) ) :
						...
						foreach ( $this->prepared_templates as $campaign_template_slug => $campaign_template_data ) :
							...
							<div class="charitable-template-list-container-item charitable-template-<?php echo esc_attr( $campaign_template_slug ); ?>">
		...
		private function prepare_templates_data() {
			...
			$templates['templates']['lite-to-pro'] = array(
				'meta'     => array(
					'label'         => 'Don\'t See Yours?',
					'description'   => 'Let us know what templates to add in the future!',
					'template_type' => 'lite-to-pro',
					'search_tags'   => array(
						'default',
						'test',
					),
				...
				'templates'  => array(
					'simple-1-col'        => array(
						'meta'     => array(
							...
							'template_type' => array( 'single', 'simple' ),
							'search_tags'   => array(
								'default',
								'simple',
								'blank',
